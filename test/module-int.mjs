...
const kbd = await import('../src/systems/keyboard/index.ts');
const brushShortcuts = await import('../src/core/brush-library/brushShortcutRegistry.ts');
const { setTool } = await import('../src/core/tools.ts');
...
const { SHAPE_SNAP_MS } = await import('../src/config/timings.ts');
const { ZOOM_MIN } = await import('../src/config/limits.ts');
...
const { drawBrushCursor } = await import('../src/systems/render/cursor.ts');
...
const swipe = await import('../src/core/swipe-actions.ts');
...
const { makeDropGap } = await import('../src/ui/dragdrop/DropGap.ts');
const { dragGhost } = await import('../src/ui/dragdrop/DragGhost.ts');
...
const fxlogic = await import('../src/logic/layer-effects.ts');
...
const status = await import('../src/systems/status.ts');
...
const { showMenuAt, showMenuForAnchor } = await import('../src/ui/dom/ShellDom.ts');
const { floatingWindow, nextFloatingZ } = await import('../src/ui/windows/FloatingWindow.ts');
...
t("module-int case 028", () => { let ran = 0; actions.replace('tool.pencil', () => ran++);
  assert.ok(kbd.handle(ev('KeyB'))); assert.equal(ran, 1); });
t("module-int case 029", () => { assert.equal(kbd.handle(ev('KeyJ')), false); });
t("module-int case 030", () => { let ran = 0; actions.replace('edit.undo', () => ran++);
  kbd.rebind('b', 'edit.undo'); kbd.handle(ev('KeyB')); assert.equal(ran, 1); kbd.resetKeymap();
  actions.replace('edit.undo', history.doUndo); actions.replace('edit.redo', history.doRedo); });
t("module-int case 031", () => { assert.equal(kbd.handle(ev('KeyB', { target: document.createElement('input') })), false); });
t("module-int brush shortcut", () => { let selected = null;
  actions.registerOrReplace('brush.selectById', (id) => { selected = id; });
  brushShortcuts.setBrushShortcuts({ lineart: '1' });
  assert.ok(kbd.handle(ev('Digit1'))); assert.equal(selected, 'lineart');
  brushShortcuts.setBrushShortcuts({});
});
...
await import('../src/systems/draw/quickshape.js'); const { QUICKSHAPE } = await import('../src/config/quickshape.ts');
...
await ta('layer PNG uses one file while folder PNG uses one tree session', async () => {
  ...
  const original = FORMATS.png.encode, seen = [], saved = [], writes = [];
  ...
  try {
    assert.equal((await xpipe.exportTargetPng(layer, false,
      { saveOutput: async (output) => saved.push(output) })).name, 'Hero Colors.png');
    S.selFolder = 1; S.markedFolders = new Set([1]);
    assert.equal((await xpipe.exportTargetPng(folder, false,
      { writerFactory })).items.length, 2);
  } finally { FORMATS.png.encode = original; }
  assert.equal(saved.length, 1);
  assert.deepEqual(writes, [['b.png'], ['c.png']]);
  assert.deepEqual(seen, [[6, 6, 'Hero Colors'], [6, 6, 'b'], [6, 6, 'c']]);
});
...
t("module-int case 200", () => {
  const opened = []; actions.unregister('ui.brushLibrary');
  actions.register('ui.brushLibrary', (mode) => { opened.push(mode); });
...
t('layer context menu exposes only the two selection-aware PNG actions', () => {
  exportProject(); layers.mount(); document.getElementById('lay-pop').classList.add('on'); layList();
  const calls = []; actions.registerOrReplace('export.targetPng',
    (target, tight) => calls.push([target, tight]));
  ...
  assert.equal(document.getElementById('lctx-png-tree'), null);
  document.getElementById('lctx-png-full').click();
  document.getElementById('lctx-png-tight').click();
  assert.deepEqual(calls, [[S.folders[0], false], [S.folders[0], true]]);
  S.cur = 0; S.marked = new Set([1]); layList();
  const layer = document.querySelector('#lay-list .lrow[data-li="1"]');
  layer.dispatchEvent(new window.MouseEvent('contextmenu',
    { bubbles: true, cancelable: true, clientY: 120 }));
  assert.equal(S.cur, 0); assert.deepEqual([...S.marked], [1]);
});
...
await ta("module-int case 292", async () => {
  const panels = await import('../src/ui/shell/PanelOrderPresenter.ts');
