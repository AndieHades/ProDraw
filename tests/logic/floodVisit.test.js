...
describe('scanline flood visitor', () => {
  it('matches the legacy region while allowing in-place paint', () => {
    ...
    grid[3][3] = [9, 9, 9];
    const expected = floodRegion(gridFloodSurface(grid), 2, 2).map(([x, y]) => y * 7 + x).sort((a, b) => a - b);
    const actual = [];
    visitFloodRegion(gridFloodSurface(grid), 2, 2, () => true, (x, y) => {
  ...
  it('honours a selection predicate', () => {
    ...
    const points = [];
    visitFloodRegion(gridFloodSurface(grid), 2, 2,
      (x, y) => x >= 1 && x <= 3 && y >= 1 && y <= 3,
