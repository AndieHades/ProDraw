...
### Глубина Undo после `Q2B-2`

`historyCap` по площади холста удалён. Вытеснение считает фактические байты
записи: тайловый патч — обе стороны каждого тайла, pixel-патч — удержанные
ячейки или плотный снимок, полный снимок документа — по одной плотной сетке на
слой. Пределы прежние и берутся из `RASTER_LIMITS`: `256 MiB` и `100` записей.

| Документ | Было | Стало |
| --- | ---: | ---: |
| `1920×1080`, 60 штрихов | `8` шагов | `60` шагов |
| `1920×1080`, 140 штрихов | `8` шагов | `100` шагов (потолок записей) |
