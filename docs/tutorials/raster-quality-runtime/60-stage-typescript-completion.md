...
`89` из оставшихся `236` JS-модулей импортируют `src/ui` — как правило `$`, `t`
и `toast` из `ui/dom/ShellDom`. Правило `no-ui-in-core-runtime` запрещает это
для TypeScript, поэтому такой модуль нельзя просто перетипизировать: он либо
переезжает в `ui`, либо получает DOM и i18n как зависимости. Это не обход
правила, а работа, которую правило и должно было заставить сделать; она
удваивает объём `Q6` и учитывается в оценке.
