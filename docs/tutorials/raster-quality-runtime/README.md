...
- Last checks: `npm run validate` зелёный; `199`/`596` TypeScript и `16`/`57`
  ...
  Повторная приёмка кистей `D9`: [Lineart, задержки пера и свойства](../../project/pen-brush-response-repair-plan.md).
  Код и визуальные/browser-проверки выполнены; физический Huion ещё не проверен.
