...
| `Q2A-4` | заливка: `logic/flood` читает ключи цвета, `draw/fill` — регион | 5 | done |
...
`logic/flood` переведён в `Q2A-4`: модуль принимает surface с `keyAt(x, y)`
вместо сетки, продакшн строит его снимком региона, а `gridFloodSurface`
сохраняет прежний контракт для чистых тестов.
...
- Commits: `Q2A-1`, `Q2A-1b`, `Q2A-2`, `Q2A-3`, `Q2A-5`, `Q2A-4`.
- Индексных чтений пикселей: `81` → `68`; предел зафиксирован храповиком
  `cutover.maximumIndexedGridReads` и может только убывать.
- Checks: `npm run validate` зелёный целиком — `167`/`469` TypeScript и
  `16`/`57` performance тестов, `435` модулей продакшн-графа.
- Осталось на `Q2B`: чтение-с-немедленной-записью в выделении, командах слоёв,
  тексте, коррекциях, эффектах и импорте — там форма ячейки уходит в историю.
- Date: 2026-09-06
