...
| --- | --- | --- |
| `Q2B-1` | симметрия считается один раз на штрих, а не на пиксель | done |
| `Q2B-2` | байтовый бюджет истории вместо `historyCap` по площади | draft |
| `Q2B-3` | снятие зеркала `grid` и удаление `sparse-grid.js` | draft |

### Что показал профиль `Q2B-1`

`62` из `65 ms` интерполированного хода приходились на накопление, `4 ms` — на
flush. Проверены и отброшены прямым замером две гипотезы: аллокация записи на
пиксель (пул объектов не изменил число) и запись в sparse `Proxy` (её полное
отключение не изменило число). Причиной был повторный расчёт симметрии.
