...
Общий multi-file export рендерит, кодирует и сохраняет один элемент за раз;
shared trim использует отдельный bounded-проход только для общей рамки. Export
UI ждёт завершения, блокирует повторный запуск и при ошибке оставляет документ
открытым с локализованным сообщением.
