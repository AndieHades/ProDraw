...
Crop/Trim сохраняют отрезанное содержимое упакованных слоёв в RGBA-строках
`PackedRasterExt`; на границе галереи они сериализуются как `rgba-ext-rows-v1`.
Старый `Map` остаётся читаемым. Обрезка, охват, автосохранение и превью не
разворачивают строки в миллионы отдельных ячеек. Превью Move/обтравки ограничено
участком, который может попасть на текущий холст. Подменю обрезки использует
общие `ToolChoiceMenus`/`ToolPanelModes`: выбранные слои либо холст по всем слоям,
включая скрытые и содержимое за краями; обе команды поддерживают точный Undo.
...
уникальные безопасные пути и последовательно пишет full-canvas PNG каждого слоя
с вложенными подпапками. Windows IPC ограничивает запись staging-каталогом и
публикует готовое дерево только после успешного завершения.
