...
export function coverageToMask(cov: CoverageMapData, size: number,
  opts: CoverageMaskOptions = {}): BinaryMask | null {
  const { threshold = 0.5, floor = 8, invert = false } = opts;
  const { w, h, data } = cov, n = Math.max(1, size | 0);
  const val = (i: number): number => invert ? 255 - (data[i] ?? 0) : data[i] ?? 0;
  ...
  for (let ty = 0; ty < th; ty++) {
    const sy0 = y0 + (ty * bh / th | 0);
    const sy1 = y0 + Math.max((ty + 1) * bh / th | 0, (ty * bh / th | 0) + 1);
    for (let tx = 0; tx < tw; tx++) {
      const sx0 = x0 + (tx * bw / tw | 0);
      const sx1 = x0 + Math.max((tx + 1) * bw / tw | 0, (tx * bw / tw | 0) + 1);
      let s = 0, c = 0;
      for (let sy = sy0; sy < sy1 && sy <= y1; sy++) {
        for (let sx = sx0; sx < sx1 && sx <= x1; sx++) { s += val(sy * w + sx); c++; } }
      const a = c ? s / c : 0; avg[ty * tw + tx] = a; if (a > max) max = a; } }
  if (max <= 0) return null;
  const out = new Uint8Array(tw * th);
  for (let i = 0; i < out.length; i++) out[i] = (avg[i] ?? 0) / max >= threshold ? 1 : 0;
  return { w: tw, h: th, data: out };
}

// наименьший период повтора бинарной сетки по оси (горизонталь/вертикаль), ≤ maxP
function period(bin: Uint8Array, w: number, h: number, maxP: number,
  ...
  for (let p = 1; p <= maxP && p < span; p++) { let ok = true;
    for (let l = 0; l < lines && ok; l++) {
      ...
      for (let k = 0; k + p < span; k++) {
        const a = horiz ? bin[fix * w + k] : bin[k * w + fix];
        const b = horiz ? bin[fix * w + k + p] : bin[(k + p) * w + fix];
...
export function coverageToTile(cov: CoverageMapData, threshold = 0.5,
  maxPeriod = 64): BinaryMask | null {
  const { w, h, data } = cov, cut = threshold * 255;
  const bin = new Uint8Array(w * h); let on = 0;
  for (let i = 0; i < bin.length; i++) {
    const v = (data[i] ?? 0) >= cut ? 1 : 0; bin[i] = v; on += v; }
  ...
  if (px && py) { const out = new Uint8Array(px * py);
    for (let y = 0; y < py; y++) for (let x = 0; x < px; x++) {
      out[y * px + x] = bin[y * w + x] ?? 0; }
    return { w: px, h: py, data: out }; }
  if (Math.max(w, h) <= maxPeriod) return { w, h, data: bin };
  const scale = maxPeriod / Math.max(w, h);
  const tw = Math.max(1, Math.round(w * scale)), th = Math.max(1, Math.round(h * scale));
  const out = new Uint8Array(tw * th);
  for (let y = 0; y < h; y++) { const ty = Math.min(th - 1, (y * th / h) | 0);
    for (let x = 0; x < w; x++) if (bin[y * w + x]) {
      out[ty * tw + Math.min(tw - 1, (x * tw / w) | 0)] = 1; } }
  let on2 = 0; for (const v of out) on2 += v;
  return on2 === 0 || on2 === out.length ? null : { w: tw, h: th, data: out };
}

// Круглый кончик size×size (дефолт для Procreate-кистей без Shape.png).
export function maskRound(size: number): BinaryMask {
  const n = Math.max(1, size | 0), out = new Uint8Array(n * n);
  const r = n / 2, c = (n - 1) / 2;
