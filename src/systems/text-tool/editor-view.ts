import type { TextSource } from "../../logic/text-model.ts";
import { lineAdvance } from "../../logic/text-layout.ts";

export interface EditorViewport {
  readonly zoom: number; readonly ox: number; readonly oy: number;
}
export interface EditorFont { readonly family: string }

export const editorText = (editor: HTMLElement): string =>
  (editor.innerText ?? editor.textContent ?? "").replace(/\n$/, "");

export function setEditorText(editor: HTMLElement, value: string): HTMLElement {
  editor.textContent = value; editor.innerText = value; return editor;
}

export function placeTextEditor(editor: HTMLElement, canvas: HTMLElement,
  src: TextSource, font: EditorFont, view: EditorViewport): void {
