...
// Ресайз окна сохраняет масштаб и панораму: точка документа под центром
// вьюпорта остаётся под центром. Раньше любой ресайз вызывал fitView.
let viewportSize = null;
const noteViewportSize = () => {
  viewportSize = { width: cv.clientWidth, height: cv.clientHeight };
};
function onViewportResize() {
  const previous = viewportSize; noteViewportSize();
  if (previous) { const next = recentredViewOffset(S.view, previous, viewportSize);
    S.view.ox = next.ox; S.view.oy = next.oy; }
  requestRender();
}
...
export function render() {
  ...
  if (z >= 7) drawGrid(ctx, 1, 1, C.grid, W, H, ox, oy, z); // обычная попиксельная сетка
  if (!S.cropMode && S.grid && (S.grid.preview || S.grid.visible)) drawGrid(ctx, S.grid.w || 16, S.grid.h || 16, gridStroke(S.grid.color, S.grid.opacity), W, H, ox, oy, z);
