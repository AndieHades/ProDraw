...
type EffectRow = HTMLElement & { __eff?: unknown };
...
function anchorIndex(rows: readonly EffectRow[]): number {
  if (S.fxCur) return rows.findIndex((r) => r.__eff === S.fxCur);
  if (S.selFolder != null) {
    return rows.findIndex((r) => Number(r.dataset["fid"]) === S.selFolder);
  }
  return rows.findIndex((r) => r.dataset["li"] != null &&
    Number(r.dataset["li"]) === S.cur);
}
...
export function selectRange(targetEl: Element): boolean {
  // фон активен — он не входит в мультивыбор; пусть будет обычный одиночный выбор
  if (S.bgSel) return false;
  const rows = [...document.querySelectorAll<EffectRow>(SEL)];
  const a = anchorIndex(rows), b = rows.indexOf(targetEl as EffectRow);
  if (a < 0 || b < 0) return false;
  S.bgSel = false; S.marked = new Set(); S.markedFolders = new Set();
  S.fxSel = new Set();
  for (let k = Math.min(a, b); k <= Math.max(a, b); k++) {
    const r = rows[k]; if (!r) continue;
    if (r.dataset["li"] != null) S.marked.add(Number(r.dataset["li"]));
    else if (r.dataset["fid"] != null) S.markedFolders.add(Number(r.dataset["fid"]));
  ...
  // активный слой — вне marked
  else { S.marked.delete(S.cur); S.selFolder = null; S.fxCur = null; }
  return true;
}
