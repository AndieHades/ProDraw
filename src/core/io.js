...
export function showSaveOverlay(u) { document.getElementById('ovlimg').src = u; document.getElementById('ovl').classList.add('on'); }
...
export async function saveFile(b, name, mime, desc, overlayUrl = null) {
  if (window.showSaveFilePicker && !matchMedia('(pointer: coarse)').matches) {
    try { const ext = '.' + name.split('.').pop();
      ...
      const w = await h.createWritable(); await w.write(b); await w.close(); bus.emit('feedback', t('toast.savedAs', { name: h.name })); return; }
