...
import { bakeGrid, clipGridToAlpha, drawBoundedGrid,
  drawEffectGrid, pixelEffects } from './layer-bake-grid.js';
...
function folderAdjustmentsForLayer(i, root) { const out = [];
  for (const folder of folderChain(S.layers[i].fid).slice().reverse()) {
    if (folderChain(folder.id).some((item) => item.id === root.id))
      out.push(...visibleAdjustments(folderEffectsFor(folder)));
  }
  return out;
}
...
function folderGroupGrid(f, selected, root) { const out = blank(S.W, S.H);
  for (let i = 0; i < S.layers.length; i++) {
    ...
    drawBoundedGrid(out, layerGrid(i, folderAdjustmentsForLayer(i, root)),
      L.opacity, S.W, S.H);
  }
  return out;
}
    ...
    if (effVis(i) && L.opacity > 0) {
      if (L.clip) {
        ...
        if (cb >= 0 && selected.has(cb) && effVis(cb)) g = clipGridToAlpha(
          layerGrid(i, folderAdjustmentsForLayer(i, f)), S.layers[cb].grid, S.W, S.H);
