...
import { fontCssFamily, fontExtension, isSupportedFontFile, resolveFont,
  safeFontName, sortFontRecords } from '../logic/FontRegistry.ts';
...
async function folderFonts() {
  ...
  for (let i = 0; i < entries.length; i++) {
    const [path, url] = entries[i], name = safeFontName(path.split('/').pop() || 'Font'),
      format = fontExtension(path);
    try {
      const data = await fetch(url).then((r) => r.arrayBuffer());
      out.push({ id: 'folder-' + name.toLowerCase().replace(/[^a-z0-9]+/g, '-'), name, family: '"' + fontCssFamily(name) + '"',
...
async function runtimeFolderFonts() {
  const files = await readAppFiles('fonts', isSupportedFontFile), out = [];
  for (let i = 0; i < files.length; i++) {
    const f = files[i], name = safeFontName(f.name), order = 40 + i;
    try { out.push({ id: 'runtime-' + name.toLowerCase().replace(/[^a-z0-9]+/g, '-'), name, family: '"' + fontCssFamily(name) + '"',
      format: fontExtension(f.name), builtin: false, folder: true, order, createdAt: 0, updatedAt: 0, data: await f.arrayBuffer() }); } catch (e) {}
  }
  return out;
}
...
export async function registerFont(f) {
  ...
  try {
    const face = new FontFace(fontCssFamily(f.id), f.data.slice(0));
    await face.load(); document.fonts.add(face);
    f.family = '"' + fontCssFamily(f.id) + '"';
  } catch (e) {}
  return f;
}
...
export async function loadFonts() {
  ...
  const list = sortFontRecords([...TEXT_FONTS.map(clone), ...(await folderFonts()),
    ...(await runtimeFolderFonts()), ...(await imported())]);
...
export function fontById(id, fonts = cache || TEXT_FONTS) {
  return resolveFont(id, fonts);
}

export async function importFontFile(file) {
  const format = fontExtension(file.name);
  ...
  const rec = { id, name: safeFontName(file.name), family: '"' + fontCssFamily(id) + '"', format, builtin: false, order, createdAt: order, updatedAt: order, data };
...
export async function renameFont(id, name) {
  ...
  if (!f) return false;
  f.name = safeFontName(name); f.updatedAt = Date.now();
